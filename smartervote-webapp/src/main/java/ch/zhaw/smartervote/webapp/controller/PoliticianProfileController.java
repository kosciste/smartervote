...
public class PoliticianProfileController {
    ...
    public String showPolitician(@PathVariable("id") String id, Model model, HttpServletRequest request) {
        ...
        model.addAttribute("personalQuestionForm", new PersonalQuestionDTO());
     ...
     *
     * @param id id of the politician
     * @param personalQuestionDTO personal question VO object that was filled by thymeleaf form bindings
    ...
    public String addQuestion(@PathVariable("id") String id,
                              PersonalQuestionDTO personalQuestionDTO,
                              RedirectAttributes redirectAttributes) {
        if (personalQuestionDTO.getText() == null || "".equals(personalQuestionDTO.getText())) {
            messageUtil.createErrorMessage(redirectAttributes, ERROR_MESSAGE_EMPTY_QUESTION);
            return "redirect:/politician/" + id;
        } else if (personalQuestionDTO.getText().length() > PersonalQuestionService.MAX_QUESTION_LENGTH) {
            messageUtil.createErrorMessage(redirectAttributes, ERROR_MESSAGE_QUESTION_TOO_LONG);
            return "redirect:/politician/" + id;
        }

        try {
            personalQuestionService.addQuestion(UUID.fromString(id), personalQuestionDTO.getText());
